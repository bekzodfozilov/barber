...
from handlers.data import RuWriteData, RuCheckData
...
Персонал <b>Usta Abdulloh</b> обладает высокой квалификацией и опытом и стремится предоставить своим клиентам наилучший сервис.", parse_mode="HTML")
    await message.answer("Введите своё имя. Мы хотели бы спросить вас кое о чем!\
 То есть мы просим вас отправлять боту свое полное имя без отправки всяких цифр и букв.", reply_markup=types.ReplyKeyboardRemove())
...
async def Ru_Username_Process(message: types.Message, state: FSMContext):
    ...
    await message.answer("Чтобы наши барберы связались с вами, пожалуйста, отправьте свой номер телефона, нажав на кнопку «📱»", reply_markup=Ru_Contact_Button)
...
@UABarbershop.message_handler(lambda message: message.text not in ["Детская стрижка", "Стандарт", "Продвинутая стрижка", "Свадебная стрижка"], state=Condition.Ru_Haircuts)
...
async def Ru_Oclock_Process(message: types.Message, state: FSMContext):
    ...
    if await RuCheckData(state):
        ...
        return await message.answer("Это место уже занято. Выберите незабронированное и удобное время, используя список выше👆")
    ...
    await bot.send_message(chat_id="@UzPythonTelegramBotsGroup", text=md.text("📝Данные клиента;\
                                                ...
                                                "\n🗓Число и месяц:", md.text(data["Ru_DM"],
                                                "\n💇🏻‍♂️Прическа:", md.text(data["Ru_Haircuts"],
                                                "\n⏰Время:", md.text(data["Ru_Oclock"])))))), reply_markup=Confirmed)
